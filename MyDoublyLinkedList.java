/*Program: MyDoublyLinkedList
 *Description: This program implements the "TBI" methods of a DoublyLinkedList class.
 *By: Gabe Skorski
 *Date: 11.24.2017
 */
...
public class MyDoublyLinkedList {
   ...
   public void insert(int n, int spot) {
	   Node toInsertAt = new Node(n);
	   
	   if (list.head.data == spot){
		 insert(n);	   
	   } else {
		   find(spot);
		   if (list.cursor == null) return;
		   list.cursor.prev.next = toInsertAt;
		   toInsertAt.prev = list.cursor.prev;
		   toInsertAt.next = list.cursor;
		   list.cursor.prev = toInsertAt;
	   }
